...
if(in_array(module_employee_department_list,$modules)){ 
	$id_admin=$_SESSION['empid'];
	$level=$_SESSION['access_level'];
	if(($level!='Super Admin') && ($level!='Admin'))
	{
		if(!isAccessModule($id_admin, module_employee_department_list))
		{
...
function ActionScript(act)
{
	...
	{
		url="popups/employee_department.php?act="+act+"&departmentID="+document.getElementById("departmentID").value;
		open_modal(url,700,550,"EMPLOYEE DEPARTMENTS")
		return true;
	}
}
...
?>
<div class="page_head">
<div id="navigation"><a href="index.php">Home</a><a> Administration</a><span style="color: #000000;">Employee Department List</span></div>


	
<form name="records" action="pages.php?src=employee_department_list.php" method="POST" onsubmit="return validatedept(this);">
<table width="100%" cellspacing="1" class="adminform1">
<tr>
	<td><h2>EMPLOYEE DEPARTMENTS</h2></td>
	<td align="right">
	<div id="option_menu">
...
Enter Employee Department Name: <input type="text" name="txtFilter" id="txtFilter" value="<?php echo $name_filter; ?>" /> 
...
$rowsPerPage=20;
	...
	$sql="SELECT  department_id, department_code, department_name, department_text, date_updated,updated_by from employee_department";
	$sql=$sql." WHERE (department_name LIKE '%".$name_filter."%') order by date_updated desc LIMIT $offset, $rowsPerPage";
...
<th colspan="18">
<?php
$sql="SELECT  COUNT(department_id) as numrows from employee_department  WHERE (department_name LIKE '%".$name_filter."%')";
